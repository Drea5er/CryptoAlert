<template>
  <div class="selectCoin">
		...
		<h3>Choose Coin for More Info </h3>
	<a class="item-link smart-select smart-select-init" data-open-in="popup"  data-searchbar="true" data-searchbar-placeholder="Search Coin">
			<select v-model="selectedCoins" name="coin3"  multiple  >
				<option v-for="(coin, index) in availableCoins" v-bind:value="coin.Name" :data-option-image="getPicURLbyIndex(index)">
				{{ coin.CoinName }}
				</option>
...
Another View
	...
	<div class="list">
  <ul >
    <li  v-for="(coin, index) in availableCoins">
      <div class="item-content">
        <div class="item-media"><img v-bind:src="getPicURLbyIndex(index)" width="44"/></div>
        <div class="item-inner">
          <div class="item-title">{{ coin.CoinName }}</div>
